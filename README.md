...
**Pons launching** (`/launch` → Pons tab) routes through `packages/adapters/src/pons/launch.ts`, which calls the real, verified `PonsV2LaunchAndBuy.launchAndBuy` contract on Robinhood Chain mainnet — not the factory's bare `launchToken`. Pons's launch contracts are open source (github.com/ponsdotdev/ponsfamily), which made this more tractable than Flap's reverse-engineering: the real `TokenParams`/`LaunchConfig` structs and every validation rule came straight from verified source, then cross-checked against a real, decoded `launchAndBuy` transaction and a real on-chain `launchFee()`/`getLaunchConfig(0)`/`canLaunch()` read. `PonsV2LaunchAndBuy` (not the factory directly) is used deliberately: its own doc comment documents a real incident — a launch's entire sellable allocation bought out by 22 addresses in the two blocks after it opened, before the creator's own follow-up buy landed — that atomic launch+buy in one transaction closes. The exact transaction this app builds was verified via a real `eth_call` simulation against live mainnet state (no revert, real predicted token/curve addresses, real token amount returned) before the UI was built — same bar as Flap. No real signed Pons launch has gone through this app yet.

**What this doesn't do yet:**
- No mainnet — see above (Pump.fun specifically; Flap and Pons are both mainnet-only, see their own sections).
...
3. **Flap — creator misattribution for vault-routed launches.** At least one real captured `TokenCreated` event recorded `creator` as the VaultPortal contract address itself, not a real wallet — VaultPortal is the on-chain caller of `Portal.newToken`, so Portal attributes the launch to it. Documented as a real data-quality risk, then fixed: `FlapAdapter.resolveRealCreator` recovers the real address via `tx.from` on the originating transaction whenever the event's creator is the known VaultPortal address — verified against the same real misattributed launch that surfaced the bug (`tx.to` was VaultPortal, `tx.from` was a genuinely different real address, and the fix's resolved `creatorAddress` matched it exactly).
...
Done since this list was last written, removed from it accordingly: real USD pricing for Pump/Flap trades, real holder/concentration tracking for Pump (via Alchemy), M1 real coin launching for Pump.fun (devnet and mainnet, with a network switch, non-custodial signing, and a real `maxSolCost` slippage guard confirmed via the actual IDL), writing app-originated launches straight into this app's own database so they show up immediately instead of waiting on the scheduled indexer, the launch/trade ordering gap in `seed-live-venue.mjs` (a trade arriving before its token's launch event used to silently skip all percentile recording for that token, forever — now caught up the moment the launch is finally recorded), and Flap's VaultPortal creator misattribution (see the bug list above).
